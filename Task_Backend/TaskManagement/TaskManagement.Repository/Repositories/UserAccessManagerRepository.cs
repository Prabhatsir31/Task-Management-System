...
namespace TaskManagement.Repository.Repositories
{
    public class UserAccessManagerRepository : IUserAccessManager
    {
        ...
        {
            var resMessage = TaskMessages.User_access_assinged_successfully;
            try
            {
                using TaskContext kUrgeTruckContext = _contextFactory.CreateKGASContext();
            ...
            catch (Exception ex)
            {
                ...
                return ResultModelFactory.CreateFailure(ResultCode.ExceptionThrown, TaskMessages.Error_while_addupdate_user_access, ex);
            }
        }
