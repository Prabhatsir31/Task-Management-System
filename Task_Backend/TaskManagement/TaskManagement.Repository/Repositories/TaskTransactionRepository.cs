...
namespace TaskManagement.Repository.Repositories
{
    public class TaskTransactionRepository : ITaskTransaction
    {
        ...
        public TaskTransactionRepository(ITaskContextFactory contextFactory,
        ...
        private void GetReportingUsers(TaskContext kUrgeTruckContext, int userId, List<UserForTaskResponse> userList)
        {
        ...
        public async Task<ResultModel> RegisterTaskAsync(TaskTransaction request)
        {
            ...
            try
            {
                ...
                //    return ResultModelFactory.CreateFailure(ResultCode.DuplicateRecord, TaskMessages.Task_Title_Exist);
                if (request.TaskId == 0)
                {
        ...
        public async Task<List<TaskTransactionResponse>> CoordinatingTeamTaskAsyn(int ParentUserId, int skipRow1, int pageSize1, int currentPage1, string searchtext1, int projectId, int assignedById, int assignedTo, int taskTypeId, string status, DateTime fromDate, DateTime toDate)
        {
            try
            {
                ...
                if (dept.Count == 0)
                {
                    // If department count is zero, return all responses from DepartmentMaster
                    var allResponses = await kUrgeTruckContext.TaskTransaction
                        .Include(x => x.ProjectMaster)
                        .Include(x => x.TaskTypeMaster)
                        .Include(x => x.UserManager)
                        .OrderByDescending(x => x.AssignedDate)
                        .Where(x => x.Title.Contains(searchtext1) || string.IsNullOrEmpty(searchtext1))
                        .Skip(skipRow1)
                        .Take(pageSize1)
                        .OrderByDescending(x => x.TaskId)
                        .ToListAsync();

                    response = _mapper.Map<List<TaskTransactionResponse>>(allResponses);
                    if (response.Count > 0)
                        response[0].TotalRecord = allResponses.Count();

                    return response;
                }

                // If department count is not zero, proceed with filtering tasks
                if (string.IsNullOrEmpty(searchtext1) == false)
                    myTask = await kUrgeTruckContext.TaskTransaction
                        .Include(x => x.ProjectMaster)
                        .Include(x => x.TaskTypeMaster)
                        .Include(x => x.UserManager)
                        .OrderByDescending(x => x.AssignedDate)
                        .Where(x => x.Title.Contains(searchtext1))
                        .ToListAsync();
                else
                    myTask = await kUrgeTruckContext.TaskTransaction
                        .Include(x => x.ProjectMaster)
        ...
        public async Task<ResultModel> ReopenTask(RepoenedRequest repoenedRequest, int raisedById)
        {
            try
            {
                ...
                resMessage = resMessage + TaskMessages.Task_reopen_successfully;
                return ResultModelFactory.UpdateSucess(resMessage);
            }
            catch (Exception ex)
            {
                Logger.Error("Error while Reopening Task " + ex);
                return ResultModelFactory.CreateFailure(ResultCode.ExceptionThrown, TaskMessages.Error_while_reopen_Task, ex);
            }
        }
        #endregion
    }
