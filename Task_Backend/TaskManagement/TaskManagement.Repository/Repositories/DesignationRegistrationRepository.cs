    ...
    public class DesignationRegistrationRepository : IDesignationRegistration
    {
        ...
        public async Task<ResultModel> RegisterDesignationAsync(DesignationMasterRequest designationRequest)
        {
            ...
            try
            {
                ...
                {
                    return ResultModelFactory.CreateFailure(ResultCode.DuplicateRecord, TaskMessages.designation_Exist);
                }
                ...
                else
                {
                    if (designationRequest.IsActive == false)
                    {
                        ...
                        if (userAssign > 0)
                            return ResultModelFactory.CreateFailure(ResultCode.Invalid, TaskMessages.design_Assign_to_User);
                    }
            ...
            catch (Exception ex)
            {
                ...
                return ResultModelFactory.CreateFailure(ResultCode.ExceptionThrown, TaskMessages.Error_while_addupdate_designation, ex);
            }
        }
    }
}
